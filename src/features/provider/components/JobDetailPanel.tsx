...
import { X, Zap, MapPin, MessageSquare, Clock, Upload } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      <div className="p-6 space-y-6">
              ...
              <div className="pt-2 border-t border-white/5">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {job.addons.map((addon) => (
                    <span key={addon} className="px-2 py-1 rounded bg-slate-800 text-xs text-slate-300">
                      {addon}
                    </span>
                  ))}
                </div>
        ...
        {job.notes && (
          <GlassCard className="p-4">
            <h3 className="text-sm font-medium text-slate-400 mb-2">Customer Notes</h3>
            ...
            {(job.status === 'in-progress' || job.status === 'completed') && (
              <div className="relative">
                ...
                <p className="text-xs text-slate-500">
                  {job.status === 'in-progress' ? 'Currently active' : 'Today, 1:45 PM'}
                </p>
              </div>
            )}
